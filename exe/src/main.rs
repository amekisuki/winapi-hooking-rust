...
use crate::instructions::{
    alloc_trampoline_mem_near_address, build_relay_function, build_trampoline,
    encode_relay_function, install_hook, steal_bytes,
};
...
fn main() {
    ...
    let mut relay_func = build_relay_function(proxy_func, 0);

    // 2) Get the length of the relay function in bytes
    let relay_func_len = encode_relay_function(&relay_func.instrs, func_addr).len();
    ...
    println!(
        "Stolen bytes {:x?}",
        encode_relay_function(&stolen_bytes.instrs, proxy_func)
    );
    println!("Bytes to steal {:?}", stolen_bytes.num_bytes);
    ...
    if stolen_bytes.num_bytes > relay_func_len {
        relay_func = build_relay_function(proxy_func, stolen_bytes.num_bytes - relay_func_len);
    }
    let relay_func_len = encode_relay_function(&relay_func.instrs, func_addr).len();
    println!("Relay function length {relay_func_len}");
...
#[no_mangle]
pub extern "system" fn get_cursor_pos_proxy(lppoint: POINT) -> BOOL {
    println!("HOOKED");
